...
export function ExperimentsPage() {
  ...
  return <div className="page experiment-page">
    ...
    {detail.data && <section className="catalog-section experiment-detail">
      ...
      <div className="experiment-detail-strip"><div><span>{t("experiment.resolved")}</span><strong>{detail.data.resolved_start ?? "—"} → {detail.data.resolved_end ?? "—"}</strong></div><div><span>{t("experiment.observations")}</span><strong>{detail.data.observation_count}</strong></div><div><span>{t("experiment.run")}</span><strong>#{detail.data.specification.attempt_number} · {detail.data.run_status}</strong></div><div><span>{t("common.quality")}</span><strong>{detail.data.specification.quality_status}</strong></div></div>
      <div className="experiment-detail-grid">
        <article><h3>{t("experiment.metrics")}</h3><div className="experiment-metrics">{detail.data.metrics.map((metric) => <div key={`${metric.series_role}-${metric.metric_key}`}><span>{metric.series_role} · {metric.name}</span><strong>{metricValue(metric)}</strong></div>)}</div></article>
        <article><h3>{t("experiment.checks")}</h3>{detail.data.quality_checks.map((check) => <div className="experiment-audit-row" key={`${check.check_key}-${check.scope_key}`}><span className={`experiment-status ${check.status}`}>{check.status}</span><div><strong>{check.check_key}</strong><p>{check.message}</p></div></div>)}</article>
