...
import { ResearchKey } from "./ResearchText";
...
export function DecisionExplorer({ resultArtifactId }: { resultArtifactId: string }) {
  ...
  return <section className="catalog-section decision-explorer">
    ...
    <div className="decision-context"><div><span>{t("decision.method")}</span><ResearchKey value={data.model_method_key} /></div><div><span>{t("decision.holdings")}</span><strong>{data.actual_holding_count} / K={data.target_k}</strong></div><div><span>{t("decision.reserve")}</span><strong>{percent(data.reserve_target_weight)}</strong></div><div><span>{t("decision.data")}</span><Link to={`/artifacts/${data.data_bundle_artifact_id}`}>{String(data.data_bundle_artifact_id).slice(0, 8)}…</Link></div></div>
    <div className="decision-assets">{data.positions.map((position) => <article className={position.selected ? "selected" : ""} key={position.asset_key}><header><div><span>{position.symbol}</span><strong>{position.asset_key}</strong></div><dl><div><dt>{t("decision.score")}</dt><dd>{number(position.model_score)}</dd></div><div><dt>{t("decision.rank")}</dt><dd>{number(position.model_rank)}</dd></div><div><dt>{t("decision.weight")}</dt><dd>{percent(position.target_weight)}</dd></div></dl></header><p className="decision-reasons"><ResearchKey value={position.decision_reason} /><ResearchKey value={position.trend_state ?? "no_trend_filter"} /></p><details open={position.selected}><summary>{t("decision.components")} ({position.components.length})</summary><div className="decision-components">{position.components.map((component) => <div key={`${position.asset_key}-${component.signal_key}`}><span><ResearchKey value={component.dimension_key} /><small>{component.signal_key}</small><small>{component.factor_variant_key}</small></span><code>{number(component.factor_value)}</code><code>{number(component.signal_score)}</code><code>{component.overall_contribution == null ? t("decision.notExact") : number(component.overall_contribution)}</code><Link aria-label={t("common.open")} to={`/artifacts/${component.signal_dataset_artifact_id}`}>→</Link></div>)}</div></details></article>)}</div>
