  ...
  return <section className={`product-ranking ${compact ? "compact" : ""}`}>
    ...
    <div className="ranking-table">
      ...
      {entries.map((entry) => <div key={entry.result_artifact_id}>
        ...
        <span><strong>{entry.model_specification_key}</strong><small>{entry.product_key}</small></span>
        <span>{entry.variant_key}<small>K={entry.target_k} · {entry.frequency}</small></span>
