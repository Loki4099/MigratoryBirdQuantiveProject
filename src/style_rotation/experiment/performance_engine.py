...
class PerformanceEngineSpec:
    ...
    def __post_init__(self) -> None:
        if self.version_number < 1 or not re.fullmatch(r"[0-9a-f]{7,64}", self.git_commit):
            raise ValueError("Performance engine requires a positive version and hex commit")
        if not re.fullmatch(r"[0-9a-f]{64}", self.dependency_lock_hash):
            raise ValueError("Performance dependency lock hash must be SHA-256")
        if not re.fullmatch(r"[0-9a-f]{64}", self.configuration_hash):
            raise ValueError("Performance configuration hash must be SHA-256")


@dataclass(frozen=True, slots=True)
class PerformanceEnginePublication:
    engine_version_id: uuid.UUID
    artifact_id: uuid.UUID
    reused: bool


def build_performance_engine_spec(
    git_commit: str, dependency_lock_path: Path, schema_revision: str, *, version_number: int = 1
) -> PerformanceEngineSpec:
    if not dependency_lock_path.is_file():
        raise ValueError(f"Dependency lock file not found: {dependency_lock_path}")
    configuration = {
        "calculator": "interval-performance-v1",
        "initialization": "carry-in-normalized-by-prior-close-nav",
        "annualization": "252-trading-days-and-actual-calendar-time-cagr",
        "risk_free": "published-dgs3mo-reserve-interval-return",
        "relative": "same-cost-strategy-and-benchmark-net-paths",
        "undefined_values": "null-with-stable-reason-code",
        "insufficient_history": "excluded-not-shortened",
    }
    return PerformanceEngineSpec(
        version_number,
        __version__,
        git_commit,
...
def publish_performance_engine(
    engine: Engine, spec: PerformanceEngineSpec
) -> PerformanceEnginePublication:
    ...
    with engine.begin() as connection:
        ...
        result = ArtifactService(cast(Engine, _BoundConnection(connection))).publish(
            ...
            artifact_key="performance_engine",
            version_number=spec.version_number,
            semantic_payload=semantic,
            content_payload=semantic,
            reason=f"publish performance_engine v{spec.version_number}",
    ...
    return PerformanceEnginePublication(engine_version_id, result.artifact_id, result.reused)
...
def _ensure_definition(connection: Connection) -> uuid.UUID:
    row = (
        connection.execute(
            text(
                "SELECT engine_definition_id, name, engine_type FROM ops.engine_definition WHERE engine_key = 'performance_engine'"
            )
        )
        .mappings()
        .one_or_none()
    ...
    if row is not None:
        if (
            row["name"] != "Deterministic Interval Performance Engine"
            or row["engine_type"] != "experiment"
        ):
            raise ValueError("Existing Performance engine definition is incompatible")
        return cast(uuid.UUID, row["engine_definition_id"])
    result = uuid.uuid4()
    connection.execute(
        text(
            "INSERT INTO ops.engine_definition (engine_definition_id, engine_key, name, engine_type) VALUES (:id, 'performance_engine', 'Deterministic Interval Performance Engine', 'experiment')"
        ),
        {"id": result},
    )
    return result
